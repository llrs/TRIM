...
for (i in seq_len(ncol(keepCol))) {
  ...
  if (ms$common != 0) {
    ...
    tryCatch(
      {
        print(plotRes(
          object = discoRes, comps = c(1, 1), what = "scores", type = "common",
          combined = TRUE, block = "1", color = "HSCT_responder"
        ) +
          lab(title = colnames(keepCol)[i]))
        print(plotRes(
          object = discoRes, comps = c(1, 1), what = "scores", type = "both",
          combined = TRUE, block = "1", color = "HSCT_responder"
        ) +
          lab(title = colnames(keepCol)[i]))
        print(plotRes(
    ...
    tryCatch(
      {
        print(plotRes(
          object = discoRes, comps = c(1, 1), what = "loadings", type = "both",
          combined = TRUE, block = "1", color = "HSCT_responder"
        ) +
          lab(title = colnames(keepCol)[i]))
        print(plotRes(
