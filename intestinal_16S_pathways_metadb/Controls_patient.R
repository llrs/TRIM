...
save(sgcca.centroid, file = "Controls.RData")
...
variables$var <- gsub("^RNAseq\\.(.*)$", "\\1", rownames(variables))
rownames(variables) <- gsub("^.*\\.(.*)$", "\\1", rownames(variables))
...
if (length(rnaseq_i) >= 2) {
  pr <- prcomp(t(gsv[rnaseq_i, ]), scale. = TRUE)
  prS <- summary(pr)
...
save(STAB, file = "bootstrap_Controls.RData")
