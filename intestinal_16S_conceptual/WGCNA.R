...
sft <- pickSoftThreshold(data.wgcna, powerVector = powers, networkType = "unsigned", RsquaredCut = 0.8)
# Plot the results:
sizeGrWindow(9, 5)
par(mfrow = c(1, 2))
cex1 <- 0.9
...
softPower <- 8
adjacency <- adjacency(data.wgcna, power = softPower, type = "unsigned")
...
sizeGrWindow(12, 9)
plot(
  geneTree, xlab = "", sub = "", main = "Gene clustering on TOM-based dissimilarity",
...
MEs <- moduleEigengenes(data.wgcna, colors = dynamicColors)$eigengenes
...
sizeGrWindow(7, 6)
plot(
  METree, main = "Clustering of module eigengenes",
  xlab = "", sub = ""
)
