...
#shrinkage
shrinkage <- c(0.301695964924249, 0, 1, 1, 1) # We guess a 0.1 for the RNAseq expression

# Basic ####
A2 <- list(RNAseq = t(expr), Micro = t(otus_table_i))
...
saveRDS(out, "models3_IBD.RDS")


models <- list.files(pattern = "models[0-9]_IBD.RDS")
...
comm + 
  geom_point(aes(color = SESCD_local))
comm + 
